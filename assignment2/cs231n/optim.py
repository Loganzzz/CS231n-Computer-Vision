...
def adam(x, dx, config=None):
    ...
    v = config['beta2'] * config['v'] + (1- config['beta2']) * (dx ** 2)  # dx**2 first
 
    config['m'] = m
    config['v'] = v

    mc = m / (1 - config['beta1'] ** config['t'])  #correct bias, only impact when first several iterations
    vc = v / (1 - config['beta2'] ** config['t'])
